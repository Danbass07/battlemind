...
class Scoreboard extends Component {
    ...
    submitHandler(e) {
        ...
        axios
            .put(`/scoreboards/${this.props.match.params.id}`, {
    ...
    getScoreboards() {
        axios
            .get(`/scoreboards/${this.props.match.params.id}/edit`)
            .then(response =>
                this.setState({
    ...
    addPlayer(player) {
        axios
            .get(
                `/scoreboards/${this.props.match.params.id}/addPlayer/${
                    player.id
                }`
            )
            .then(response =>
    ...
    removePlayer(player) {
        axios
            .get(
                `/scoreboards/${this.props.match.params.id}/removePlayer/${
                    player.id
                }`
            )
            .then(response =>
    ...
    resultChangeController(player_id, category, key) {
        ...
        axios.get(
            ...
            }/addResult/${player_id}/${category}/${this.state.action}`
        );
    }

    renderResultsDynamic(player) {
        return this.state.ScoreboardPlayers.sort(
            this.compareValues("result", "desc")
        ).map((ScoreboardPlayer, index) => (
            <div key={ScoreboardPlayer.id}>
                {this.state.ScoreboardPlayers[index].pivot &&
                this.state.ScoreboardPlayers[index].pivot.player_id ===
                    player.id ? (
                    <div>
                        <button
                            onClick={() =>
                                this.resultChangeController(
                                    player.id,
                                    "Win",
                                    index
                                )
